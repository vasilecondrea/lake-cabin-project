...
import math
from transform_helper import delete_cols_from_df, split_datetime_list_to_date_and_time_list


def create_dim_counterparty(counterparty_df, address_df):
    dim_counterparty_df = counterparty_df.copy()

    cols_to_delete = ['commercial_contact',
                      'delivery_contact', 'created_at', 'last_updated']
    dim_counterparty_df = delete_cols_from_df(
        dim_counterparty_df, cols_to_delete)
    ...
    result = pd.merge(dim_counterparty_df, address_df_copy,
                      on='legal_address_id')
...
def create_dim_transaction(transaction_df):
    ...
    dim_transaction_df['sales_order_id'] = [int(
        sales_order_id) if sales_order_id != 'None' else None for sales_order_id in dim_transaction_df['sales_order_id']]

    dim_transaction_df['purchase_order_id'] = [int(
        purchase_order_id) if purchase_order_id != 'None' else None for purchase_order_id in dim_transaction_df['purchase_order_id']]

    cols_to_delete = ['created_at', 'last_updated']
    dim_transaction_df = delete_cols_from_df(
        dim_transaction_df, cols_to_delete)
...
def create_dim_payment_type(payment_df):
    ...
    cols_to_delete = ['created_at', 'last_updated']
    dim_payment_type_df = delete_cols_from_df(
        dim_payment_type_df, cols_to_delete)
...
def create_dim_currency(currency_df, lookup):
    ...
    currency_names = [lookup[currency_code]
                      for currency_code in dim_currency['currency_code']]
...
def create_dim_date(sale_related_df):
    ...
    cols_to_extract = ['created_at', 'last_updated',
    ...
    dim_date = dim_date.astype(
        {'year': 'int', 'month': 'int', 'day': 'int', 'day_of_week': 'int', 'quarter': 'int'})
    return dim_date
...
def create_dim_staff(staff_df, department_df):
    ...
    department_df_copy = delete_cols_from_df(
        department_df_copy, cols_to_delete)
...
def create_fact_sales_order(sales_order_df):
    ...
    fact_sales_order['sales_order_id'] = [int(
        sales_order_id) if sales_order_id != 'None' else None for sales_order_id in fact_sales_order['sales_order_id']]
    fact_sales_order['created_date'] = split_datetime_list_to_date_and_time_list(
        fact_sales_order['created_at'])['dates']
    fact_sales_order['created_time'] = split_datetime_list_to_date_and_time_list(
        fact_sales_order['created_at'])['times']
    fact_sales_order['last_updated_date'] = split_datetime_list_to_date_and_time_list(
        fact_sales_order['last_updated'])['dates']
    fact_sales_order['last_updated_time'] = split_datetime_list_to_date_and_time_list(
        fact_sales_order['last_updated'])['times']
    fact_sales_order['agreed_delivery_date'] = [datetime.strptime(
        date, '%Y-%m-%d') for date in fact_sales_order['agreed_delivery_date']]
    fact_sales_order['agreed_payment_date'] = [datetime.strptime(
        date, '%Y-%m-%d') for date in fact_sales_order['agreed_payment_date']]
...
def create_fact_payment(payment_df):
    ...
    fact_payment['created_date'] = split_datetime_list_to_date_and_time_list(
        fact_payment['created_at'])['dates']
    fact_payment['created_time'] = split_datetime_list_to_date_and_time_list(
        fact_payment['created_at'])['times']
    fact_payment['last_updated_date'] = split_datetime_list_to_date_and_time_list(
        fact_payment['last_updated'])['dates']
    fact_payment['last_updated_time'] = split_datetime_list_to_date_and_time_list(
        fact_payment['last_updated'])['times']
    fact_payment['payment_date'] = [datetime.strptime(
        date, '%Y-%m-%d') for date in fact_payment['payment_date']]
...
def create_fact_purchase_order(purchase_df):
    ...
    fact_purchase_orders['created_date'] = split_datetime_list_to_date_and_time_list(
        fact_purchase_orders['created_at'])['dates']
    fact_purchase_orders['created_time'] = split_datetime_list_to_date_and_time_list(
        fact_purchase_orders['created_at'])['times']
    fact_purchase_orders['last_updated_date'] = split_datetime_list_to_date_and_time_list(
        fact_purchase_orders['last_updated'])['dates']
    fact_purchase_orders['last_updated_time'] = split_datetime_list_to_date_and_time_list(
        fact_purchase_orders['last_updated'])['times']
    fact_purchase_orders['agreed_delivery_date'] = [datetime.strptime(
        date, '%Y-%m-%d') for date in fact_purchase_orders['agreed_delivery_date']]
    fact_purchase_orders['agreed_payment_date'] = [datetime.strptime(
        date, '%Y-%m-%d') for date in fact_purchase_orders['agreed_payment_date']]

    cols_to_delete = ['created_at', 'last_updated']
    fact_purchase_orders = delete_cols_from_df(
        fact_purchase_orders, cols_to_delete)
